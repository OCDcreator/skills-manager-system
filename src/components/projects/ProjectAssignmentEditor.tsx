...
export function ProjectAssignmentEditor(props: ProjectAssignmentEditorProps) {
  ...
  return (
    <section className="grid gap-4 rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
      <div className="grid gap-4 min-[1380px]:grid-cols-2">
        <div className="flex min-h-0 flex-col overflow-hidden rounded-2xl border border-slate-800 bg-slate-950/60">
          ...
          <div className="skill-markdown-scroll min-h-0 flex-1 space-y-1 overflow-y-auto px-4 py-3">
            {props.agents.map((agent) => (
              ...
              >
                ...
                />
                <span className="min-w-0 flex-1">
                  <span className="block font-medium text-slate-100">{agent.displayName}</span>
