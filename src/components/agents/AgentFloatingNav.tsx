...
export function AgentFloatingNav({ agents }: AgentFloatingNavProps) {
      ...
      <div className="pointer-events-auto relative">
        ...
        >
          {navItems.map((item, index) => {
            ...
            return (
              <li className="relative z-10 h-14" key={item.href}>
                <a
                  className="relative block h-14 w-full text-right transition-all duration-300 ease-out focus-visible:outline-none"
                  href={item.href}
                ...
                >
                  <span
                    className={`absolute right-16 top-1/2 -translate-y-1/2 overflow-hidden whitespace-nowrap rounded-full border border-slate-700/60 bg-slate-950/88 py-2 text-sm font-semibold text-slate-100 shadow-2xl shadow-slate-950/45 backdrop-blur-md transition-all duration-300 ease-out ${
                      isActive
