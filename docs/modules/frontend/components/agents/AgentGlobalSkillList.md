...
The sidecar shell is a full-height flex column with hidden overflow, and the scrollable list uses `flex-1` instead of a fixed max-height so it fills the wrapper height without stretching the surrounding grid row. The list now remembers its per-agent scroll position across refreshes. The component never mutates files directly: it forwards explicit button clicks back to `AgentsView`, which owns confirmation dialogs and Tauri command calls.
