...
Known target groups import directly into their Agent without another selector. Generic candidates that cannot be resolved to a supported Agent are placed in the `manual` group and keep the explicit target selector before import. Candidate rows still show detection class, suggested-agent hints, source-of-truth path, direct child folders/files, and import state. Imported mirrors delegate row-level update/repair controls to `ExternalImportList` inside the same Agent group.
