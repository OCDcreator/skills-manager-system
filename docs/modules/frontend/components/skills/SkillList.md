...
The component renders a responsive card grid, highlights the selected skill, dims disabled skills, and forwards toggle/select actions to the parent. Each source window uses a generous viewport-aware height cap and keeps only the card grid internally scrollable with the shared `skill-markdown-scroll` scrollbar skin, so long custom/external buckets stay usable without feeling cramped. External skills now get additive badge treatment: unmanaged external entries show a manual-external badge, while managed GitHub mirrors show a separate managed-source badge derived from `skill.managedSource`.
