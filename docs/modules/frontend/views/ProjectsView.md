...
Multi-agent project assignment workbench for create/edit flows plus a secondary saved-project management list.
...
The view does not track a single selected agent. Instead, it derives `selectedAgentKeys` from `projectDraftAgentKeys(draft)` and uses batch toggle functions (`toggleProjectAgentSkillForAgents`, `toggleProjectAgentSceneForAgents`) to apply skill/scene changes across all selected agents in one action. Skill filtering uses `filterProjectSkillsForMultiAgent` which evaluates three-state selection (`all`/`some`/`none`) across the agent drafts.
