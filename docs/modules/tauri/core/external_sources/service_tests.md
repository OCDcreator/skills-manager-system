...
| Test | Purpose |
|---|---|
| `list_external_sources_reads_variants_from_persisted_fetch_commit` | Confirms read-only source listing uses the persisted fetched commit to discover variants, metadata, direct child directory/file previews, and matching content fingerprints without depending on worktree files or remote HEAD resolution. |
