...
| Test | Purpose |
|---|---|
| `taking_over_unmanaged_entry_keeps_existing_contents_when_sync_would_conflict` | Confirms take-over preserves manual contents even when folder-name sync logic would otherwise collide. |
| `deleting_requested_entry_preserves_other_target_entries` | Confirms delete only removes the named entry, including app-managed folder-name entries. |
