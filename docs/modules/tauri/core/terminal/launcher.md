...
- reuse `core::command_resolution::command_candidate_strings()` so terminal
  launcher candidates match the Git resolver's platform order
...
- on macOS, keep the bare executable first but also try common Homebrew and
  system absolute paths so Finder-launched GUI sessions with sparse `PATH`
  values can still find installed CLIs
