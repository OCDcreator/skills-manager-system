...
fn detect_generated_agent_bundle_supports_root_hidden_agent_roots() {
    ...
    assert!(result
        .variants
        .iter()
        .any(|variant| variant.agent_key == "codex" && variant.variant_path == ".agents/skills/impeccable"));
    ...
    assert!(result
        .variants
        .iter()
        .any(|variant| variant.agent_key == "cursor" && variant.variant_path == ".cursor/skills/impeccable"));
    assert!(result.variants.iter().any(|variant| {
