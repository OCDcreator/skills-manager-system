...
use crate::core::external_sources::{
    add_external_source_with_input as add_external_source_core,
    ...
    update_external_import as update_external_import_core, AddExternalSourceInput,
...
fn require_repo_path(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    load_repo_path(app)?.ok_or_else(|| "Repository path is not configured".to_string())
}
